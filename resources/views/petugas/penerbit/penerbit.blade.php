 ...
 @section('konten')
     ...
     <div class="my-3 p-3 bg-body rounded shadow-sm">
         <!-- FORM PENCARIAN -->
         <div class="pb-3">
             <form class="d-flex" action="{{url('penerbits')}}" method="get">
             ...
             <tbody>
                 ...
                 @foreach ($data as $item)
                     <tr>
                         ...
                         <td>{{ $item->alamat}}</td>
                         <td>
                             <a href='{{ url('penerbits/' . $item->id . '/edit') }}' class="btn btn-warning btn-sm">Edit</a>
                             <form onsubmit="return confirm('Yakin Data Akan Dihapus')" class="d-inline"
                                 action="{{ url('penerbits/' . $item->id) }}" method="post">
                                 @csrf
                                 @method('DELETE')
