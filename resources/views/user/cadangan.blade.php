...
<div class="my-3 p-3 bg-body rounded shadow-sm">
    ...
    <div class="row mt-3">
        ...
        <div class="col-md-12 mt-4 d-flex justify-content-between">
            @php
                // $item = $pinjam->where('buku', $item)->where('status', '!=', 'kembali')->first();
                $koleksiPinjam = $pinjam->firstWhere('buku', $data->id);
                // dd($item);
            @endphp

            {{-- @php
            $item = $pinjam->where('nama_buku', $data->nama_buku)
                        ->whereNotIn('status', ['kembali', 'batal'])
                        ->first();
            @endphp --}}
            ...
            {{-- @endforeach --}}
            {{-- {{dd($pinjam)}} --}}
            {{-- <a href="{{ route('account.peminjaman', $item->id) }}" class="btn btn-success">Pinjam</a>
